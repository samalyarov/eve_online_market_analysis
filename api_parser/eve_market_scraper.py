...
for system in systems.keys():
    ...
    for i in tqdm.tqdm(range(0, len(ids)+1, 200)):
        ...
        # Acquire data
        curr_market_data = requests.get(f'https://api.evemarketer.com/ec/marketstat/json?typeid={ids_string}&usesystem={system}')
        curr_market_data = curr_market_data.json()
    ...
    for column in lists_to_ints:
        try: # Add try-except for testing reasons
            data[column] = data[column].apply(lambda x:x[0])
        except:
            print(f'Cannot convert column {column} from list to integer. Skipping.')
    ...
    for column in strings_to_ints:
        try: 
            ...
            print(f'Cannot convert column {column} from string to integer. Skipping.')
    ...
    records_collected = data.shape[0]
    print(f'Inserting {records_collected} records into the database...')
    ...
    # Setting the columns (not neccessary, but handy for testing|bugfixes)
